...
abstract class Command {
    ...
    //Message Builder
    var embeddedMessageBuilder: EmbedBuilder = EmbedBuilder().setColor(Color.RED).clearFields().setTitle(null).setDescription(null)
    ...
    fun sendUsageMessage(event: MessageReceivedEvent) {
        val cmdProperties = this.getCmdProperties()
        embeddedMessageBuilder.setTitle("${cmdProperties?.name} :no_entry:")
                .addField("Usage", "``${BotProperties.DEFAULT_COMMAND_PREFIX}${cmdProperties?.usage}``", true)
        sendMessage(event)
    }
